  ...
  useEffect(() => {
    const checkIfLiked = async () => {
      try {
        ...
        const response = await fetch(`process.env.VITE_API_URL/likes/posts/${post.id}/me`, {
          headers: {
  ...
  useEffect(() => {
    const getLikeCount = async () => {
      try {
        ...
        const response = await fetch(`process.env.VITE_API_URL/likes/posts/${post.id}/count`);
...
useEffect(() => {
  const getCommentCount = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`process.env.VITE_API_URL/posts/${post.id}/comments/count`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
...
useEffect(() => {
  const fetchComments = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`process.env.VITE_API_URL/posts/${post.id}/comments`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
...
useEffect(() => {
  const fetchCurrentUser = async () => {
    ...
    try {
      const userResponse = await fetch('process.env.VITE_API_URL/users/me/profile', {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        // Unlike the post
        const response = await fetch(`process.env.VITE_API_URL/likes/posts/${post.id}/like`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const refreshComments = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const commentsResponse = await fetch(`process.env.VITE_API_URL/posts/${post.id}/comments`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteComment = async (commentId: number) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`process.env.VITE_API_URL/comments/${commentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const response = await fetch(`process.env.VITE_API_URL/posts/${post.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
